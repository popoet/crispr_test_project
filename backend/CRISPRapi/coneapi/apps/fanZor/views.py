...
from . import fanzor, models
from .tasks import run_fanzor_analysis
...
class FanZorExecuteView(APIView):
    """
    FanZor执行
    """
    ...
    def _process_request(self, inputSequence, pam, spacerLength, sgRNAModule, name_db):
        ...
        result_fanzor = models.result_fanZor_list.objects.filter(input_sequence=inputSequence,
                                                             pam_type=pam,
        ...
        if result_fanzor.first():
            task_record = result_fanzor.first()
            # 从文件中读取结果数据
        ...
        # 检查是否已存在相同但失败的任务
        failed_fanzor = models.result_fanZor_list.objects.filter(input_sequence=inputSequence,
                                                             pam_type=pam,
                                                             spacer_length=spacerLength,
                                                             sgRNA_module=sgRNAModule,
                                                             name_db=name_db,
                                                             task_status="failed")
        if failed_fanzor.exists():
            # 生成任务标识符用于重试计数
            task_identifier = f"fanzor_{inputSequence}_{pam}_{spacerLength}_{sgRNAModule}_{name_db}"
            ...
            # 自动清理所有失败的任务并重新执行
            for failed_task in failed_fanzor:
        ...
        # 检查是否已存在正在进行中或等待中的任务
        existing_fanzor = models.result_fanZor_list.objects.filter(input_sequence=inputSequence,
                                                               pam_type=pam,
                                                               spacer_length=spacerLength,
                                                               sgRNA_module=sgRNAModule,
                                                               name_db=name_db)
        pending_or_running_fanzor = existing_fanzor.filter(task_status__in=['pending', 'running'])
        if pending_or_running_fanzor.exists():
            # 返回最新的任务状态信息
            latest_task = pending_or_running_fanzor.latest('submit_time')
            # 简单估算：假设任务需要5分钟完成
        ...
        # 创建任务记录
        fanzor_task_record = models.result_fanZor_list(
            task_id=task_id,
        ...
        # 异步执行任务
        run_fanzor_analysis.delay(task_id, inputSequence, pam, spacerLength, sgRNAModule, name_db)
    ...
    def _safe_cleanup_task(self, task_id):
        ...
        with _task_cleanup_lock:
            try:
                # 删除数据库记录
                try:
                    task_record = models.result_fanZor_list.objects.get(task_id=task_id)
                    task_record.delete()
                except models.result_fanZor_list.DoesNotExist:
                    # 记录不存在，但继续尝试清理文件
                    pass
                
                # 删除工作目录
                task_work_dir = os.path.join(settings.BASE_DIR, 'work', 'fanZorTasks', task_id)
                task_tmp_dir = os.path.join(settings.BASE_DIR, 'work', 'fanZorTmp', task_id)
    ...
    def _cleanup_orphaned_data(self, input_sequence, pam_type, spacer_length, sgRNA_module, name_db):
        ...
        with _task_cleanup_lock:
            # 查找可能存在的孤立记录
            orphaned_records = models.result_fanZor_list.objects.filter(
            ...
            for record in orphaned_records:
                ...
                # 检查工作目录是否存在
                task_work_dir = os.path.join(settings.BASE_DIR, 'work', 'fanZorTasks', record.task_id)
                task_tmp_dir = os.path.join(settings.BASE_DIR, 'work', 'fanZorTmp', record.task_id)
    ...
    def _get_retry_info(self, input_sequence, pam_type, spacer_length, sgRNA_module, name_db):
        ...
        获取重试相关信息
        """
        task_identifier = f"fanzor_{input_sequence}_{pam_type}_{spacer_length}_{sgRNA_module}_{name_db}"
        retry_count = cache.get(task_identifier, 0)
        return {
            "retry_count": retry_count,
            "max_retries": MAX_RETRY_COUNT,
...
class FanZorJbrowseAPI(APIView):
    def get(self, request):
        task_id = request.query_params.get('task_id')
        ...
        try:
            fanzor_task_record = models.result_fanZor_list.objects.get(task_id=task_id)
        except models.result_fanZor_list.DoesNotExist:
            return HttpResponseNotFound("Task not found")

        fa = os.path.join(settings.BASE_DIR, f"database/TargetGenome/{fanzor_task_record.name_db}/{fanzor_task_record.name_db}.fa")
        fai = fa + ".fai"
        gff = os.path.join(tmp_dir, f"{fanzor_task_record.name_db}_{task_id}_sgRNA.gff3")
        gff_gz = gff + ".gz"
        gff_csi = gff_gz + ".csi"
        gff_tbi = gff_gz + ".tbi"
        
        # 基因轨道文件路径
        gene_gff = os.path.join(tmp_dir, f"{fanzor_task_record.name_db}_{task_id}_genes.gff3")
        ...
        # ========== 生成 GFF ==========
        result_file = os.path.join(settings.BASE_DIR, fanzor_task_record.sgRNA_with_JBrowse_json)
