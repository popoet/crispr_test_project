...
def setup_task_logger(task_path, task_id):
    ...
    # 创建任务特定的日志记录器
    logger_name = f"fanzor_task_{task_id}"
...
def form2Database(task_id, inputSequence, pam, spacerLength, sgRNAModule, name_db):
    ...
    try:
        ...
        try:
            ...
            fanzor_task_record = result_fanZor_list(task_id=task_id, input_sequence=inputSequence, pam_type=pam,
        ...
        if fasta_sequence:
            fanzor_task_record.sequence_position = fasta_sequence_position_json
            fanzor_task_record.save()
        else:
            fanzor_task_record.task_status = 'failed'
            fanzor_task_record.log = 'empty BLAST result'
            fanzor_task_record.save()
        ...
        if len(unique_gene_ids) > 2:
            fanzor_task_record.task_status = 'failed'
            fanzor_task_record.log = f"错误：目标区域包含 {len(unique_gene_ids)} 个不同基因（{unique_gene_ids.tolist()}），超过最大允许数量（2）。"
            fanzor_task_record.save()
