...
from . import crisprA, models
from .tasks import run_crispra_analysis
...
class CrisprAExecuteView(APIView):
    """
    CrisprA执行
    """
    ...
    def post(self, request):
        ...
        upstream_sequence_length = request.data.get('upstream_sequence_length', 2000)
        
        # 使用全局锁防止同一参数的并发执行
        task_key = f"{inputSequence}_{pam}_{spacerLength}_{sgRNAModule}_{name_db}_{upstream_sequence_length}"
        with _task_execution_lock:
            return self._process_request(inputSequence, pam, spacerLength, sgRNAModule, name_db, upstream_sequence_length)

    def _process_request(self, inputSequence, pam, spacerLength, sgRNAModule, name_db, upstream_sequence_length):
        """
        实际处理CrisprA请求的核心逻辑
        包含完整的任务重试和并发处理逻辑
        """
        # 检查数据库中是否已存在相同且已完成的任务
        result_crispra = models.result_crispra_list.objects.filter(input_sequence=inputSequence,
                                                             pam_type=pam,
                                                             spacer_length=spacerLength,
                                                             sgRNA_module=sgRNAModule,
                                                             upstream_sequence_length=upstream_sequence_length,
                                                             name_db=name_db,
                                                             task_status="finished",
                                                             sgRNA_with_JBrowse_json__isnull=False)
        if result_crispra.first():
            task_record = result_crispra.first()
            # 从文件中读取结果数据
        ...
        # 检查是否已存在相同但失败的任务
        failed_crispra = models.result_crispra_list.objects.filter(input_sequence=inputSequence,
                                                             pam_type=pam,
                                                             spacer_length=spacerLength,
                                                             sgRNA_module=sgRNAModule,
                                                             upstream_sequence_length=upstream_sequence_length,
                                                             name_db=name_db,
                                                             task_status="failed")
        if failed_crispra.exists():
            # 生成任务标识符用于重试计数
            task_identifier = f"crispra_{inputSequence}_{pam}_{spacerLength}_{sgRNAModule}_{name_db}_{upstream_sequence_length}"
            ...
            # 自动清理所有失败的任务并重新执行
            for failed_task in failed_crispra:
        ...
        # 检查是否已存在正在进行中或等待中的任务
        existing_crispra = models.result_crispra_list.objects.filter(input_sequence=inputSequence,
                                                               pam_type=pam,
                                                               spacer_length=spacerLength,
                                                               sgRNA_module=sgRNAModule,
                                                               upstream_sequence_length=upstream_sequence_length,
                                                               name_db=name_db)
        pending_or_running_crispra = existing_crispra.filter(task_status__in=['pending', 'running'])
        if pending_or_running_crispra.exists():
            # 返回最新的任务状态信息
            latest_task = pending_or_running_crispra.latest('submit_time')
            # 简单估算：假设任务需要5分钟完成
            ...
            return Response({
                ...
                "retry_info": self._get_retry_info(inputSequence, pam, spacerLength, sgRNAModule, name_db, upstream_sequence_length)
            }, status=status.HTTP_202_ACCEPTED)

        # 在创建新任务前再次清理可能的孤立数据
        self._cleanup_orphaned_data(inputSequence, pam, spacerLength, sgRNAModule, name_db, upstream_sequence_length)

        # 验证参数
        try:
            spacerLength = int(spacerLength)
            upstream_sequence_length = int(upstream_sequence_length)
        ...
        # 创建任务记录
        crispra_task_record = models.result_crispra_list(
            task_id=task_id,
        ...
        # 异步执行任务
        run_crispra_analysis.delay(task_id, inputSequence, pam, spacerLength, sgRNAModule, name_db, upstream_sequence_length)
    ...
    def _safe_cleanup_task(self, task_id):
        ...
        with _task_cleanup_lock:
            try:
                # 删除数据库记录
                try:
                    task_record = models.result_crispra_list.objects.get(task_id=task_id)
                    task_record.delete()
                except models.result_crispra_list.DoesNotExist:
                    # 记录不存在，但继续尝试清理文件
                    pass
                
                # 删除工作目录
                task_work_dir = os.path.join(settings.BASE_DIR, 'work', 'crisprATasks', task_id)
                task_tmp_dir = os.path.join(settings.BASE_DIR, 'work', 'crisprATmp', task_id)
    ...
    def _cleanup_orphaned_data(self, input_sequence, pam_type, spacer_length, sgRNA_module, name_db, upstream_sequence_length):
        """
        清理孤立的数据（有记录但无文件，或有文件但无记录）
        """
        with _task_cleanup_lock:
            # 查找可能存在的孤立记录
            orphaned_records = models.result_crispra_list.objects.filter(
            ...
            for record in orphaned_records:
                ...
                # 检查工作目录是否存在
                task_work_dir = os.path.join(settings.BASE_DIR, 'work', 'crisprATasks', record.task_id)
                task_tmp_dir = os.path.join(settings.BASE_DIR, 'work', 'crisprATmp', record.task_id)
    ...
    def _get_retry_info(self, input_sequence, pam_type, spacer_length, sgRNA_module, name_db, upstream_sequence_length):
        """
        获取重试相关信息
        """
        task_identifier = f"crispra_{input_sequence}_{pam_type}_{spacer_length}_{sgRNA_module}_{name_db}_{upstream_sequence_length}"
        retry_count = cache.get(task_identifier, 0)
        return {
            "retry_count": retry_count,
            "max_retries": MAX_RETRY_COUNT,
...
class CrisprAJbrowseAPI(APIView):
    def get(self, request):
        task_id = request.query_params.get('task_id')
        ...
        try:
            crispra_task_record = models.result_crispra_list.objects.get(task_id=task_id)
        except models.result_crispra_list.DoesNotExist:
            return HttpResponseNotFound("Task not found")

        fa = os.path.join(settings.BASE_DIR, f"database/TargetGenome/{crispra_task_record.name_db}/{crispra_task_record.name_db}.fa")
        fai = fa + ".fai"
        gff = os.path.join(tmp_dir, f"{crispra_task_record.name_db}_{task_id}_sgRNA.gff3")
        gff_gz = gff + ".gz"
        gff_csi = gff_gz + ".csi"
        gff_tbi = gff_gz + ".tbi"
        
        # 基因轨道文件路径
        gene_gff = os.path.join(tmp_dir, f"{crispra_task_record.name_db}_{task_id}_genes.gff3")
        ...
        # ========== 生成 GFF ==========
        result_file = os.path.join(settings.BASE_DIR, crispra_task_record.sgRNA_with_JBrowse_json)
        ...
        if file_type == "gff3.gz.csi":
            ...
            if os.path.exists(gene_gff_gz):
                return FileResponse(open(gene_gff_gz, "rb"))
            return HttpResponseNotFound("Gene track file missing")
        else:
