...
def setup_task_logger(task_path, task_id):
    ...
    # 创建任务特定的日志记录器
    logger_name = f"crispra_task_{task_id}"
...
def form2Database(task_id, inputSequence, pam, spacerLength, sgRNAModule, name_db, upstream_sequence_length=2000):
    conda_env_path = settings.CONDA_ENV_PATH
    # 确保应用的任务目录和临时目录存在
    tasks_base_dir = os.path.join(settings.BASE_DIR, 'work', 'crisprATasks')
    tmp_base_dir = os.path.join(settings.BASE_DIR, 'work', 'crisprATmp')
    os.makedirs(tasks_base_dir, exist_ok=True)
    os.makedirs(tmp_base_dir, exist_ok=True)
    
    task_path = os.path.join(settings.BASE_DIR, 'work', 'crisprATasks', str(task_id))
    ...
    try:
        ...
        try:
            ...
            crispra_task_record = result_crispra_list(task_id=task_id, input_sequence=inputSequence, pam_type=pam,
                                                spacer_length=spacerLength, sgRNA_module=sgRNAModule, name_db=name_db,
                                                task_status='running', sequence_position='', upstream_sequence_length=upstream_sequence_length)
            crispra_task_record.save()

        # 1、识别输入序列类型
        input_type = input_sequence_to_fasta_sequence_position(inputSequence, task_logger)
        # 3、获取序列和位置信息（针对CRISPRa，如果输入的是基因ID，则获取其上游区域）
        fasta_sequence, fasta_sequence_position = input_type_to_sequence_and_position_with_upstream(
            task_path, blastb_software, input_type, fa_path, gff_pkl_path, task_id, upstream_sequence_length, task_logger)
        fasta_sequence_position_json = json.dumps(fasta_sequence_position)
        if fasta_sequence:
            crispra_task_record.sequence_position = fasta_sequence_position_json
            crispra_task_record.save()
        else:
            crispra_task_record.task_status = 'failed'
            crispra_task_record.log = 'empty BLAST result'
            crispra_task_record.save()
        ...
        if len(unique_gene_ids) > 2:
            crispra_task_record.task_status = 'failed'
            crispra_task_record.log = f"错误：目标区域包含 {len(unique_gene_ids)} 个不同基因（{unique_gene_ids.tolist()}），超过最大允许数量（2）。"
            crispra_task_record.save()
        ...
        crispra_task_record.sgRNA_json = guide_json
    ...
    try:
        ...
        json_handle = {"TableData": {"json_data": guide_json},
                       "JbrowseInfo": {
                           "assembly": {
                               "name": name_db,
                               "fasta": f"{settings.ADDR}/api/crisprA/crispra_Jbrowse_API/?task_id={task_id}&file_type=fa",
                               "fai": f"{settings.ADDR}/api/crisprA/crispra_Jbrowse_API/?task_id={task_id}&file_type=fai"
                           },
                           "tracks": {
                               "name": name_db,
                               "gff3_gz": f"{settings.ADDR}/api/crisprA/crispra_Jbrowse_API/?task_id={task_id}&file_type=gff3.gz",
                               "gff3_tbi": f"{settings.ADDR}/api/crisprA/crispra_Jbrowse_API/?task_id={task_id}&file_type=gff3.gz.csi"
                           },
...
def input_type_to_sequence_and_position_with_upstream(task_path, blastb_software, input_type, fa_path, gff_pkl_path, task_id, upstream_sequence_length, task_logger=None):
    """
    获取序列和位置信息，对于基因ID类型的输入，获取其上游指定长度的区域
    """
    try:
        import os
        import pandas as pd
        ...
        if input_type['position']:
            ...
            return sequence, blastnfmt6_100_dict
        if input_type['locus']:
            # 对于基因ID，获取其上游upstream_sequence_length长度的区域
            genome_handle = pysam.FastaFile(fa_path)
            ...
            # 计算上游区域的起始和结束位置
            upstream_start = max(1, start - upstream_sequence_length)  # 不超过染色体起点
            upstream_end = start  # 上游区域结束于基因起始位置
            
            # 提取上游序列
            sequence = genome_handle.fetch(seqid, upstream_start, upstream_end)
            return sequence, {"seqid": seqid, "start": upstream_start, "end": upstream_end}
        return 0, 1
    except Exception as e:
        if task_logger:
