...
def run_tnpb_analysis(self, task_id, inputSequence, pam, spacerLength, sgRNAModule, name_db):
    ...
    try:
        ...
        if response_data == "未能获取序列和位置信息":
            ...
            try:
                logger.info(f"开始生成TNPB任务 {task_id} 的GFF文件")
                
                result_file = os.path.join(settings.BASE_DIR, response_data)
                if not os.path.exists(result_file):
                    logger.error(f"TNPB任务 {task_id} 的结果文件不存在: {result_file}")
                else:
                    ...
                    logger.info(f"TNPB任务 {task_id} sgRNA GFF生成完成")
                    
                    # === 生成基因 GFF ===
                    gene_gff = os.path.join(tmp_dir, f"{name_db}_{task_id}_genes.gff3")
                    gene_gff_gz = gene_gff + ".gz"
                    
                    from .views import TnpBJbrowseAPI
                    view_instance = TnpBJbrowseAPI()
                    ...
                    logger.info(f"TNPB任务 {task_id} 基因GFF生成完成")
                    
            except Exception as e:
                logger.warning(f"TNPB任务 {task_id} GFF文件生成失败: {str(e)}")
