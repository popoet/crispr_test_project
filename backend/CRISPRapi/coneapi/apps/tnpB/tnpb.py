...
def setup_task_logger(task_path, task_id):
    ...
    # 创建任务特定的日志记录器
    logger_name = f"tnpb_task_{task_id}"
...
def form2Database(task_id, inputSequence, pam, spacerLength, sgRNAModule, name_db):
    ...
    # 确保应用的任务目录和临时目录存在
    tasks_base_dir = os.path.join(settings.BASE_DIR, 'work', 'tnpBTasks')
    tmp_base_dir = os.path.join(settings.BASE_DIR, 'work', 'tnpBTmp')
    os.makedirs(tasks_base_dir, exist_ok=True)
    os.makedirs(tmp_base_dir, exist_ok=True)
    
    task_path = os.path.join(settings.BASE_DIR, 'work', 'tnpBTasks', str(task_id))
    ...
    try:
        ...
        try:
            ...
            tnpb_task_record = result_tnpB_list(task_id=task_id, input_sequence=inputSequence, pam_type=pam,
        ...
        if fasta_sequence:
            tnpb_task_record.sequence_position = fasta_sequence_position_json
            tnpb_task_record.save()
        else:
            tnpb_task_record.task_status = 'failed'
            tnpb_task_record.log = 'empty BLAST result'
            tnpb_task_record.save()
        ...
        if len(unique_gene_ids) > 2:
            tnpb_task_record.task_status = 'failed'
            tnpb_task_record.log = f"错误：目标区域包含 {len(unique_gene_ids)} 个不同基因（{unique_gene_ids.tolist()}），超过最大允许数量（2）。"
            tnpb_task_record.save()
        ...
        json_file_path = add_Jbrowse_to_json(task_id, task_path, fasta_sequence_position_json, guide_json, name_db, 
                                             gene_ids=unique_gene_ids.tolist() if len(unique_gene_ids) > 0 else None,
                                             input_sequence=inputSequence, task_logger=task_logger)
...
def add_Jbrowse_to_json(task_id, task_path, sequence_position, guide_json, name_db, gene_ids=None, input_sequence=None, task_logger=None):
    try:
        tnpb_task_record = result_tnpB_list.objects.get(task_id=task_id)
        sequence_position = json.loads(tnpb_task_record.sequence_position)
        
        tracks_config = {
            "name": f"{name_db}_sgRNAs",
            "gff3_gz": f"{settings.ADDR}/api/tnpB/tnpb_Jbrowse_API/?task_id={task_id}&file_type=gff3.gz",
            "gff3_tbi": f"{settings.ADDR}/api/tnpB/tnpb_Jbrowse_API/?task_id={task_id}&file_type=gff3.gz.csi"
        }
        
        if gene_ids:
            if input_sequence is None:
                input_sequence = tnpb_task_record.input_sequence
            ...
            tracks_config["genes_gff3_gz"] = f"{settings.ADDR}/api/tnpB/tnpb_Jbrowse_API/?task_id={task_id}&file_type=genes.gff3.gz"
            tracks_config["genes_gff3_tbi"] = f"{settings.ADDR}/api/tnpB/tnpb_Jbrowse_API/?task_id={task_id}&file_type=genes.gff3.gz.csi"
        ...
        json_handle = {"task_id": task_id,
                       ...
                       "JbrowseInfo": {
                           "assembly": {
                               "name": name_db,
                               "fasta": f"{settings.ADDR}/api/tnpB/tnpb_Jbrowse_API/?task_id={task_id}&file_type=fa",
                               "fai": f"{settings.ADDR}/api/tnpB/tnpb_Jbrowse_API/?task_id={task_id}&file_type=fai"
                           },
