...
class IscBExecuteView(APIView):
    ...
    def post(self, request):
        ...
        if result_iscb.first():
            # 从文件中读取结果数据
            result_file_path = os.path.join(settings.BASE_DIR, result_iscb.first().sgRNA_with_JBrowse_json)
        ...
        failed_iscb = models.result_iscB_list.objects.filter(input_sequence=inputSequence,
                                                             ...
                                                             name_db=name_db,
                                                             task_status="failed")
        if failed_iscb.first():
            # 返回错误信息，提醒用户重新提交
            return Response({
                "msg": "任务之前执行失败",
                "error": failed_iscb.first().log,
                "suggest": "请重新提交任务"
        ...
        if pending_or_running_iscb.first():
            # 返回任务正在处理中的信息和预估完成时间
            task_record = pending_or_running_iscb.first()
            # 简单估算：假设任务需要5分钟完成
