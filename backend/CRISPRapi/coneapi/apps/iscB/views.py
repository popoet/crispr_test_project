...
class IscBJbrowseAPI(APIView):
    def get(self, request):
        ...
        # 基因轨道文件路径
        gene_gff = os.path.join(tmp_dir, f"{iscb_task_record.name_db}_{task_id}_genes.gff3")
    ...
    def _generate_gene_gff(self, result_file, gene_gff_path, name_db):
        ...
        try:
            ...
            print(f"DEBUG: 需要提取的基因 ID 数量: {len(gene_ids)}")
            
            gene_records_df = self._extract_genes_from_gff(gff_path, gene_ids)
            print(f"DEBUG: 成功提取到 {len(gene_records_df)} 条基因记录")
