...
def setup_task_logger(task_path, task_id):
    ...
    # 创建任务特定的日志记录器
    logger_name = f"iscb_task_{task_id}"
...
def form2Database(task_id, inputSequence, pam, spacerLength, sgRNAModule, name_db):
    ...
    # 确保应用的任务目录和临时目录存在
    tasks_base_dir = os.path.join(settings.BASE_DIR, 'work', 'iscBTasks')
    tmp_base_dir = os.path.join(settings.BASE_DIR, 'work', 'iscBTmp')
    os.makedirs(tasks_base_dir, exist_ok=True)
    os.makedirs(tmp_base_dir, exist_ok=True)
    
    task_path = os.path.join(settings.BASE_DIR, 'work', 'iscBTasks', str(task_id))
    ...
    try:
        ...
        try:
            ...
            iscb_task_record = result_iscB_list(task_id=task_id, input_sequence=inputSequence, pam_type=pam,
        ...
        if fasta_sequence:
            iscb_task_record.sequence_position = fasta_sequence_position_json
            iscb_task_record.save()
        else:
            iscb_task_record.task_status = 'failed'
            iscb_task_record.log = 'empty BLAST result'
            iscb_task_record.save()
        ...
        if len(unique_gene_ids) > 2:
            iscb_task_record.task_status = 'failed'
            iscb_task_record.log = f"错误：目标区域包含 {len(unique_gene_ids)} 个不同基因（{unique_gene_ids.tolist()}），超过最大允许数量（2）。"
            iscb_task_record.save()
...
def add_Jbrowse_to_json(task_id, task_path, sequence_position, guide_json, name_db, task_logger=None):
    try:
        iscb_task_record = result_iscB_list.objects.get(task_id=task_id)
        sequence_position = json.loads(iscb_task_record.sequence_position)
        ...
        tracks_config = {
            ...
            "gff3_gz": f"{settings.ADDR}/api/iscB/iscb_Jbrowse_API/?task_id={task_id}&file_type=gff3.gz",
            "gff3_tbi": f"{settings.ADDR}/api/iscB/iscb_Jbrowse_API/?task_id={task_id}&file_type=gff3.gz.csi"
        }
        
        if gene_ids:
            input_sequence = iscb_task_record.input_sequence
            ...
            tracks_config["genes_gff3_gz"] = f"{settings.ADDR}/api/iscB/iscb_Jbrowse_API/?task_id={task_id}&file_type=genes.gff3.gz"
            tracks_config["genes_gff3_tbi"] = f"{settings.ADDR}/api/iscB/iscb_Jbrowse_API/?task_id={task_id}&file_type=genes.gff3.gz.csi"
        ...
        json_handle = {"task_id": task_id,
                       ...
                       "JbrowseInfo": {
                           "assembly": {
                               "name": name_db,
                               "fasta": f"{settings.ADDR}/api/iscB/iscb_Jbrowse_API/?task_id={task_id}&file_type=fa",
                               "fai": f"{settings.ADDR}/api/iscB/iscb_Jbrowse_API/?task_id={task_id}&file_type=fai"
                           },
