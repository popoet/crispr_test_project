...
def setup_task_logger(task_path, task_id):
    ...
    # 创建任务特定的日志记录器
    logger_name = f"crispr_epigenome_task_{task_id}"
...
def form2Database(task_id, inputSequence, pam, spacerLength, sgRNAModule, name_db):
    ...
    # 确保应用的任务目录和临时目录存在
    tasks_base_dir = os.path.join(settings.BASE_DIR, 'work', 'crisprEpigenomeTasks')
    tmp_base_dir = os.path.join(settings.BASE_DIR, 'work', 'crisprEpigenomeTmp')
    os.makedirs(tasks_base_dir, exist_ok=True)
    os.makedirs(tmp_base_dir, exist_ok=True)
    
    task_path = os.path.join(settings.BASE_DIR, 'work', 'crisprEpigenomeTasks', str(task_id))
    ...
    try:
        ...
        try:
            ...
            crispr_epigenome_task_record = result_crispr_epigenome_list(task_id=task_id, input_sequence=inputSequence, pam_type=pam,
        ...
        # 对于CRISPR Epigenome，我们需要找到CpG岛区域
        if fasta_sequence and fasta_sequence_position:
            # 保存原始位置信息
            cpg_island_info['original_position'] = dict(fasta_sequence_position)
            
            # 查找CpG岛区域
            cpg_island_region = find_cpg_island_region(fa_path, fasta_sequence_position, task_logger)
            if cpg_island_region and 'is_original_region' not in cpg_island_region:
                # 找到了CpG岛区域
                cpg_island_info['found'] = True
                cpg_island_info['cpg_island_position'] = cpg_island_region
                cpg_island_info['message'] = f"在输入区域附近找到CpG岛区域: {cpg_island_region['seqid']}:{cpg_island_region['start']}-{cpg_island_region['end']}"
                
                # 更新序列和位置信息为CpG岛区域
                fasta_sequence_position = cpg_island_region
                seqid = cpg_island_region['seqid']
                start = cpg_island_region['start']
                end = cpg_island_region['end']
                with pysam.FastaFile(fa_path) as genome_handle:
                    fasta_sequence = genome_handle.fetch(seqid, start, end)
                    
                task_logger.info(cpg_island_info['message'])
            else:
                # 没有找到CpG岛区域，使用原始区域
                cpg_island_info['found'] = False
                cpg_island_info['message'] = "未在输入区域附近找到符合条件的CpG岛区域，将直接在输入区域设计"
                task_logger.info(cpg_island_info['message'])
        
        fasta_sequence_position_json = json.dumps(fasta_sequence_position)
        cpg_island_info_json = json.dumps(cpg_island_info)
        
        if fasta_sequence:
            crispr_epigenome_task_record.sequence_position = fasta_sequence_position_json
            # 存储CpG岛相关信息到log字段（仅用于错误信息）
            crispr_epigenome_task_record.log = cpg_island_info_json
            crispr_epigenome_task_record.save()
        else:
            crispr_epigenome_task_record.task_status = 'failed'
            crispr_epigenome_task_record.log = 'empty BLAST result'
            crispr_epigenome_task_record.save()
        ...
        # 添加检查确保 family_records 不为空且包含所需列
        if family_records is None or family_records.empty:
            task_logger.warning("No gene family information was found in the target region. (It is possible that a CpG region was identified, but no gene family information is available for it.)")
            unique_gene_ids = []
            
            # 如果找到了CpG岛区域但没有找到基因家族信息，则回退到原始区域查找
            if cpg_island_info.get('found', False) and cpg_island_info.get('original_position'):
                task_logger.info("在CpG岛区域未找到基因家族信息，回退到原始输入区域查找")
                original_position = cpg_island_info['original_position']
                original_family_records = target_to_ontarget(gff_path, gff_pkl_path, 
                                                           original_position['seqid'], 
                                                           original_position['start'], 
                                                           original_position['end'], 
                                                           task_logger)
                
                if original_family_records is not None and not original_family_records.empty:
                    family_records = original_family_records
                    task_logger.info("在原始输入区域找到基因家族信息")
                    task_logger.info(family_records)
                    
                    # 重新计算unique_gene_ids
                    if 'featuretype' in family_records.columns:
                        gene_rows = family_records[family_records['featuretype'] == 'gene']
                        unique_gene_ids = gene_rows['ID'].drop_duplicates().values if not gene_rows.empty else []
                        task_logger.info(f"在原始区域检测到的唯一基因 ID: {unique_gene_ids.tolist() if hasattr(unique_gene_ids, 'tolist') else unique_gene_ids}")
                        
                        # 更新fasta_sequence_position为原始区域，以便后续步骤使用
                        fasta_sequence_position = original_position
                        # 重新生成目标序列
                        ...
                        # 更新CpG岛信息，标明在CpG岛区域未找到基因家族信息
                        cpg_island_info['message'] += " (在CpG岛区域未找到基因家族信息，已回退到原始输入区域)"
                        cpg_island_info_json = json.dumps(cpg_island_info)
                        crispr_epigenome_task_record.log = cpg_island_info_json
                        crispr_epigenome_task_record.save()
                else:
                    task_logger.warning("在原始输入区域也未找到基因家族信息")
        else:
            # 检查是否存在 featuretype 列
            if 'featuretype' not in family_records.columns:
                task_logger.error(f"family_records 缺少 featuretype 列，可用列: {family_records.columns.tolist()}")
                unique_gene_ids = []
            else:
                # === 提取唯一的 gene ID ===
        ...
        task_logger.info(
            f"检测到的唯一基因 ID: {unique_gene_ids.tolist() if hasattr(unique_gene_ids, 'tolist') else unique_gene_ids}")

        # Step 3: 判断数量
        if len(unique_gene_ids) > 2:
            crispr_epigenome_task_record.task_status = 'failed'
            crispr_epigenome_task_record.log = f"错误：目标区域包含 {len(unique_gene_ids)} 个不同基因（{unique_gene_ids.tolist() if hasattr(unique_gene_ids, 'tolist') else list(unique_gene_ids)}），超过最大允许数量（2）。"
            crispr_epigenome_task_record.save()
        ...
        # 9、添加 JBrowse 可视化信息并保存到文件，包括CpG岛信息
        json_file_path = add_Jbrowse_to_json(task_id, task_path, fasta_sequence_position_json, guide_json, name_db, cpg_island_info, task_logger)
...
# 三个条件
def find_cpg_island_region(fa_path, original_position, task_logger=None):
    """
    查找CpG岛区域
    CpG岛定义:
    1. 长度 >= 200 bp
    ...
    """
    try:
        seqid = original_position['seqid']
        original_start = original_position['start']
        original_end = original_position['end']

        # 扩展搜索区域，查找附近的CpG岛
        search_start = max(1, original_start - 5000)  # 向上游扩展5kb
        search_end = original_end + 5000  # 向下游扩展5kb

        with pysam.FastaFile(fa_path) as genome_handle:
            sequence = genome_handle.fetch(seqid, search_start, search_end).upper()

        # 滑动窗口查找CpG岛
        window_size = 200
        step_size = 50

        best_cpg_island = None
        best_score = 0
        ...
        for i in range(0, len(sequence) - window_size + 1, step_size):
            ...
            expected_cpg = (window_seq.count('C') * window_seq.count('G')) / len(window_seq)
            cpg_ratio = cpg_count / expected_cpg if expected_cpg > 0 else 0

            # 检查是否满足CpG岛条件
            if len(window_seq) >= 200 and gc_content >= 0.5 and cpg_ratio >= 0.6:
                # 计算得分（综合考虑GC含量和CpG比率）
                score = gc_content * cpg_ratio

                # 检查是否与原始区域有重叠或接近
                window_start = search_start + i
                window_end = search_start + i + window_size

                # 计算与原始区域的距离
                if window_end < original_start:
                    distance = original_start - window_end
                elif window_start > original_end:
                    distance = window_start - original_end
                else:
                    distance = 0  # 有重叠

                # 距离越近得分越高
                distance_score = 1 / (1 + distance/1000)
                final_score = score * distance_score
        ...
        if best_cpg_island and best_score > 0:
            if task_logger:
                task_logger.info(f"找到CpG岛区域: {best_cpg_island}")
            return best_cpg_island
        else:
            # 如果没有找到CpG岛，则返回原始区域，并添加标记
            if task_logger:
                task_logger.info("未找到CpG岛区域，使用原始区域")
            original_position_with_flag = dict(original_position)
            original_position_with_flag['is_original_region'] = True
            return original_position_with_flag

    except Exception as e:
        if task_logger:
            task_logger.error(f"查找CpG岛区域时发生错误: {str(e)}", exc_info=True)
        # 出错时返回原始区域，并添加标记
        original_position_with_flag = dict(original_position) if original_position else {}
        original_position_with_flag['is_original_region'] = True
        return original_position_with_flag
# 两个条件
# def find_cpg_island_region(fa_path, original_position, task_logger=None):
...
#             gc_content = gc_count / len(window_seq)
#
#             # 检查是否满足CpG岛条件（仅检查长度和GC含量）
#             if len(window_seq) >= 200 and gc_content >= 0.5:
#                 # 计算得分（仅基于GC含量）
#                 score = gc_content
#
#                 # 检查是否与原始区域有重叠或接近
...
#                     }
#
#         if best_gc_region and best_score > 0:
#             if task_logger:
#                 task_logger.info(f"找到高GC区域: {best_gc_region}")
#             return best_gc_region
#         else:
#             # 如果没有找到高GC区域，则返回原始区域，并添加标记
#             if task_logger:
#                 task_logger.info("未找到高GC区域，使用原始区域")
#             original_position_with_flag = dict(original_position)
#             original_position_with_flag['is_original_region'] = True
#             return original_position_with_flag
#
#     except Exception as e:
#         if task_logger:
#             task_logger.error(f"查找高GC区域时发生错误: {str(e)}", exc_info=True)
#         # 出错时返回原始区域，并添加标记
#         original_position_with_flag = dict(original_position) if original_position else {}
#         original_position_with_flag['is_original_region'] = True
#         return original_position_with_flag
...
def add_Jbrowse_to_json(task_id, task_path, sequence_position, guide_json, name_db, cpg_island_info=None, task_logger=None):
    try:
        crispr_epigenome_task_record = result_crispr_epigenome_list.objects.get(task_id=task_id)
        sequence_position = json.loads(crispr_epigenome_task_record.sequence_position)

        # 获取CpG岛信息
        cpg_island_info = {}
        try:
            cpg_island_info = json.loads(crispr_epigenome_task_record.log)
