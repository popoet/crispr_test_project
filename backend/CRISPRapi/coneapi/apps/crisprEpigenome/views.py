...
class CrisprEpigenomeExecuteView(APIView):
    ...
    #   {
    #     "inputSequence": "Ghjin_A01:20000-21000",
    #     "pam": "NGG",
    #     "spacerLength": "20",
    ...
    def _process_request(self, inputSequence, pam, spacerLength, sgRNAModule, name_db):
        ...
        # 创建任务记录
        crispr_epigenome_task_record = models.result_crispr_epigenome_list(
            task_id=task_id,
        ...
        # 异步执行任务
        run_crispr_epigenome_analysis.delay(task_id, inputSequence, pam, spacerLength, sgRNAModule, name_db)
...
class CrisprEpigenomeJbrowseAPI(APIView):
    def get(self, request):
        ...
        try:
            crispr_epigenome_task_record = models.result_crispr_epigenome_list.objects.get(task_id=task_id)
        except models.result_crispr_epigenome_list.DoesNotExist:
            return HttpResponseNotFound("Task not found")

        fa = os.path.join(settings.BASE_DIR, f"database/TargetGenome/{crispr_epigenome_task_record.name_db}/{crispr_epigenome_task_record.name_db}.fa")
        fai = fa + ".fai"
        gff = os.path.join(tmp_dir, f"{crispr_epigenome_task_record.name_db}_{task_id}_sgRNA.gff3")
        gff_gz = gff + ".gz"
        gff_csi = gff_gz + ".csi"
        gff_tbi = gff_gz + ".tbi"
        
        # 基因轨道文件路径
        gene_gff = os.path.join(tmp_dir, f"{crispr_epigenome_task_record.name_db}_{task_id}_genes.gff3")
        ...
        # ========== 生成 GFF ==========
        result_file = os.path.join(settings.BASE_DIR, crispr_epigenome_task_record.sgRNA_with_JBrowse_json)
