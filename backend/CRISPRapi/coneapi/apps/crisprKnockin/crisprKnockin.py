...
def setup_task_logger(task_path, task_id):
    ...
    # 创建任务特定的日志记录器
    logger_name = f"crisprknockin_task_{task_id}"
...
def form2Database(task_id, inputSequence, pam, spacerLength, sgRNAModule, name_db):
    ...
    # 确保应用的任务目录和临时目录存在
    tasks_base_dir = os.path.join(settings.BASE_DIR, 'work', 'crisprKnockinTasks')
    tmp_base_dir = os.path.join(settings.BASE_DIR, 'work', 'crisprKnockinTmp')
    os.makedirs(tasks_base_dir, exist_ok=True)
    os.makedirs(tmp_base_dir, exist_ok=True)
    
    task_path = os.path.join(settings.BASE_DIR, 'work', 'crisprKnockinTasks', str(task_id))
    ...
    try:
        ...
        try:
            ...
            crisprknockin_task_record = result_crisprknockin_list(task_id=task_id, input_sequence=inputSequence, pam_type=pam,
        ...
        if fasta_sequence:
            crisprknockin_task_record.sequence_position = fasta_sequence_position_json
            crisprknockin_task_record.save()
        else:
            crisprknockin_task_record.task_status = 'failed'
            crisprknockin_task_record.log = 'empty BLAST result'
            crisprknockin_task_record.save()
        ...
        if len(unique_gene_ids) > 2:
            crisprknockin_task_record.task_status = 'failed'
            crisprknockin_task_record.log = f"错误：目标区域包含 {len(unique_gene_ids)} 个不同基因（{unique_gene_ids.tolist()}），超过最大允许数量（2）。"
            crisprknockin_task_record.save()
