...
def form12aDatabase(task_id, inputSequence, pam, spacerLength, sgRNAModule, name_db):
    ...
    # 确保应用的任务目录和临时目录存在
    tasks_base_dir = os.path.join(settings.BASE_DIR, 'work', 'cas12aTasks')
    tmp_base_dir = os.path.join(settings.BASE_DIR, 'work', 'cas12aTmp')
...
def form12bDatabase(task_id, inputSequence, pam, spacerLength, sgRNAModule, name_db):
    conda_env_path = settings.CONDA_ENV_PATH
    # 确保应用的任务目录和临时目录存在
    tasks_base_dir = os.path.join(settings.BASE_DIR, 'work', 'cas12bTasks')
    tmp_base_dir = os.path.join(settings.BASE_DIR, 'work', 'cas12bTmp')
    os.makedirs(tasks_base_dir, exist_ok=True)
    os.makedirs(tmp_base_dir, exist_ok=True)
